...
void save_rs2points_to_pcd (window& app, rs2::points& points, rs2::video_frame& color, std::string fn_tmstmp, int id)
{
    if (!points)
        return;
    std::cout << "Saving pointcloud as PCD..." << std::endl;
    // convert from realsense cloud to PCL pointcloud
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud = PCL_Conversion(points, color);
    // save pointcloud as .pcd
    std::string new_folder = "../../pcd/pointclouds_from_" + fn_tmstmp;
    if (!boost::filesystem::is_directory(new_folder))
        boost::filesystem::create_directory(new_folder);
    std::string cloud_filename = new_folder + "/" + std::to_string(id) + ".pcd";
    pcl::io::savePCDFileASCII(cloud_filename, *cloud);
    app.save_as_pcd = false;
    std::cout << "Successfully saved!" << std::endl;
}

void save_cvMat_to_png (cv::Mat image, std::string fn_tmstmp, int id)
{
    std::string new_folder = "../../pcd/pointclouds_from_" + fn_tmstmp;
    if (!boost::filesystem::is_directory(new_folder))
        boost::filesystem::create_directory(new_folder);
    std::string image_filename = new_folder + "/" + std::to_string(id) + ".png";
    cv::imwrite(image_filename, image);
}

void save_camera_intrinsics (rs2_intrinsics intrinsics, std::string fn_tmstmp)
{
    // if folder of current run doesn't exist, create it
    std::string new_folder = "../../pcd/pointclouds_from_" + fn_tmstmp;
    if (!boost::filesystem::is_directory(new_folder))
        boost::filesystem::create_directory(new_folder);
    // exit function if camera intrinsics were already saved
    std::string json_dict_filename = new_folder + "/camera_intrinsics.json";
    if (boost::filesystem::exists(json_dict_filename))
        return;
    
    // store camera intrinsics in JSON object
    json camint;
    camint["width"] = intrinsics.width;
    camint["height"] = intrinsics.height;
    camint["ppx"] = intrinsics.ppx;
    camint["ppy"] = intrinsics.ppy;
    camint["fx"] = intrinsics.fx;
    camint["fy"] = intrinsics.fy;
    camint["model"] = intrinsics.model;
    camint["coeffs"] = intrinsics.coeffs;

    // save JSON object to a file
    std::ofstream o(json_dict_filename);
    o << std::setw(2) << camint << std::endl;
}

void save_aligned_depth_frame_to_png(rs2::frameset frameset, std::string fn_tmstmp, int id)
{
    // if folder of current run doesn't exist, create it
    std::string new_folder = "../../pcd/pointclouds_from_" + fn_tmstmp;
    if (!boost::filesystem::is_directory(new_folder))
    ...
    // cv::Mat depth_img(cv::Size(w, h), CV_16UC1, (void*)aligned_depth_frame.get_data(), cv::Mat::AUTO_STEP);

    // // Create OpenCV matrix of size (w,h) from the colorized depth data
    // const int wi = color_frame.as<rs2::video_frame>().get_width();
    // const int he = color_frame.as<rs2::video_frame>().get_height();
    // cv::Mat color_img(cv::Size(wi, he), CV_8UC3, (void*)color_frame.get_data(), cv::Mat::AUTO_STEP);

    // // aligned_depth_frame show
    // cv::imshow("Test", depth_img);
    // cv::waitKey(110000); 
